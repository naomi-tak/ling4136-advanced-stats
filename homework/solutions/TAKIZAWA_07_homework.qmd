...
pacman::p_load(rstudioapi, tidyverse, lme4, brms, bayestestR, tidybayes, bayesplot)
...
# bmrs(Bayesian Regression Models using Stan) formula
quant_formula <- bf(log_rt ~ log_frequency+
                     (1 + log_frequency | PartID) +
                     (1 | ItemID))
# check priors
get_prior(quant_formula, quantese) # intercept, log_frequency, sigma?

# specify weakly informative priors
my_priors <- c(prior(normal(6.5, 2.5), class = "Intercept"),
               prior(normal(0, 2.5), class = "b", coef = "log_frequency"))
...
my_priors <- c(my_priors, 
               prior(lkj(1), class = 'cor')) #LKJ = Lewandowski-Kurowicka-Joe
# lkj(1) : assuming any correlation can equally possible (up to +- 1)
# lkj(2) : closer to 0, prior assuming weaker correlation
# lkj(3) : even more close to 0, prior assuming almost no correlation
```
...
mcmc_plot(xmdl_quant, type = "trace") # looks good, caterpillar
mcmc_dens_overlay(xmdl_quant, pars = "b_log_frequency") # looks good
xmdl_quant # Bulk ESS is too low
...
mcmc_plot(xmdl_quant, type = "trace")
mcmc_dens_overlay(xmdl_quant, pars = "b_log_frequency") # looks good
xmdl_quant # ESS fine
...
# create new df with the continuous predicdtor
min <- min(quantese$log_frequency)
max <- max(quantese$log_frequency)

new_data <- tibble(
  log_frequency=seq(min, max, length.out=100)) #delt på 100 
