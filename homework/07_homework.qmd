...
# check priors
get_prior(mdl_formula, quantese) # intercept, log_frequency, sigma?

# prior exploration
quantese %>% 
  ggplot(aes(x=log_rt, y=log_frequency))+
  geom_point()+
  theme_minimal()

# - for intercept 
min_rt <- min(quantese$log_rt) # 5.87
max_rt <- max(quantese$log_rt) # 7.79
mean_rt <- mean(quantese$log_rt, na.rm=TRUE) # 6.54
sd_rt <- (max_rt - min_rt)/4

#plot_normal(mean_rt, sd_rt)

# - for coef log_freq -- outcome(rt) sd / predictor(freq) sd
min_freq <- min(quantese$log_frequency) 
max_freq <- max(quantese$log_frequency) 
mean_freq <- mean(quantese$log_frequency, na.rm=TRUE) 
sd_freq <- (max_freq - min_freq)/4

#plot_normal(mean_freq, sd_freq)

sd_coef <- sd_rt/sd_freq

# specify weakly informative priors
my_priors <- c(prior(normal(mean_rt, sd_rt), class = "Intercept"),
               prior(normal(0, sd_coef), class = "b", coef = "log_frequency"))

```

We can also now get used to specifying priors for classes `sd` and `cor`. Priors of [class `sd`]{.underline} are priors for the variance associated with our random effects. You see here 2 `sd` priors for group `ItemID`. The first one is just a generic prior for all `ItemID` coefficients and the second one with the `coef = Intercept` stands for the random intercept variation between items. Similarly for group `PartID`, we have 3 priors. The first one is again just a generic prior for all `PartID` coefficients. The second one with the `coef = Intercept` stands for the random intercept variation between participants. And the last one with the `coef = log_frequency` stands for the random slope variation between participants for the predictor `log_frequency`. How do we specify these priors. We have usually no idea what to expect from these parameters, so we here also specify very diffuse priors centered on zero. We pick a cauchy distribution (remember? super squashed normal distributions) because they have thick tails. Let's make them super diffuse and pick a `sd = 3` for both items and participants.
...
# full model
xmdl <- brm(log_rt ~ log_frequency + (1 + log_frequency | PartID) + (1 | ItemID),
                  family = gaussian(),
                  prior = my_priors,
                  data = quantese,
                  chains = 4, cores = 4,
            iter = 1000, warmup = 1000, seed = 12)

# I couldnt find a way to fix the following error, 

```

![](images/clipboard-1725270259.png)
